...
function _PP_ver4(idx, vars, config)
    K, T, Ang, Phi, ExtKidx = vars
    para, kgrid, n = config.userdata
    k = K[1]
    dim, β, me, λ, μ, e0, ϵ0 = para.dim, para.β, para.me, para.mass2, para.μ, para.e0, para.ϵ0
    ...
    ϵ = dot(k1, k1) / (2me) - μ
    G2dd = ElectronLiquid.Propagator.green_derive(τ4 - τ2, ϵ, β, 0)
    G2sd = ElectronLiquid.Propagator.green_derive(τ4 - τ1, ϵ, β, 0)
    G2ds = ElectronLiquid.Propagator.green_derive(τ3 - τ2, ϵ, β, 0)
    G2ss = ElectronLiquid.Propagator.green_derive(τ3 - τ1, ϵ, β, 0)
    q = extK1 - k
    qabs = sqrt(dot(q, q))
    V1d = ElectronLiquid.interactionDynamic(para, qabs, τ1, τ2)
    V1s = ElectronLiquid.interactionStatic(para, qabs, τ1, τ2)

    q = k - extK3
    qabs = sqrt(dot(q, q))
    V2d = ElectronLiquid.interactionDynamic(para, qabs, τ3, τ4)
    V2s = ElectronLiquid.interactionStatic(para, qabs, τ3, τ4)
    l1 = n[1]
    l2 = n[2]
    l3 = n[3]
    factor = 1.0 / (2π)^3
    factor_legend = sin(θ) / (4π)
    fdd = phase((τ1, τ3, τ2, τ4), l1, l2, l3, β) * factor * factor_legend
    fsd = phase((τ1, τ3, τ1, τ4), l1, l2, l3, β) * factor * factor_legend
    fds = phase((τ1, τ3, τ2, τ3), l1, l2, l3, β) * factor * factor_legend
    fss = phase((τ1, τ3, τ1, τ3), l1, l2, l3, β) * factor * factor_legend
    return G1 * (G2dd *V1d*V2d*fdd + G2ds *V1d*V2s*fds + G2sd *V1s*V2d*fsd + G2ss *V1s*V2s*fss) *para.NF
end

function _measure_ver4(idx, vars, obs, weight, config) # for the mcmc algorithm
    # K, T, N, Ang, ExtKidx = vars
    # ki = ExtKidx[1]
    ki = vars[5][1]
    obs[idx][ki] += weight
...
function aLambda_benchmark(para::ParaMC;
    kamp=[para.kF],
    n=[0, 0, 0], 
    neval=1e6,
    config=nothing,
    ...
    result = integrate(integrand; measure=_measure_ver4, config=config, solver=:mcmc, neval=neval, kwargs...)
    ...
    if isnothing(result) == false
        ...
        ver4 = r
        for (ki, k) in enumerate(kamp)
            println("k=$(k/para.kF)kF\t ver4=$(ver4[ki])\n")
        end
        # println("ver3=$(ver3)")
